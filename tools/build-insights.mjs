...
function main() {
  ...
  const perScheme = schemes.map((scheme) => {
    ...
    return {
      ...
      scheme_group: scheme.scheme_group,
  ...
  const insights = {
    ...
    agriculture_count: schemes.filter((s) => s.scheme_group === 'agriculture').length,
    related_welfare_count: schemes.filter((s) => s.scheme_group === 'related_welfare').length,
