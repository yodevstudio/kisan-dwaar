// Pure, environment-agnostic scheme-record validation — docs/DATA_SPEC.md
// §1 (record schema) and §3 (subsidy_rule schema). No Node imports here on
// purpose: this file is shared by tools/validate-data.mjs (the Node CLI,
// K3's data-entry gate) and S5's CMS (running in a browser), so a record
// passes identically in both places or the whole point of "the CMS
// validates before publish" is hollow. Whoever obtains docs/DATA_SPEC.md's
// text — fs.readFileSync in Node, fetch() in the browser — hands it to
// parseRatePolicyVocabulary below; this file never touches the filesystem
// or the network itself.

export const SCHEME_ID_RE = /^RJ_[A-Z0-9_]+$/;
...
// Parses the §4 rate_policy table out of already-obtained DATA_SPEC.md
// text. Kept separate from *how* that text was obtained so the CLI and
// the browser CMS share this exact parser and can never silently diverge
// on what "the §4 table" means.
export function parseRatePolicyVocabulary(specText) {
  const section = specText.match(/## §4[^\n]*\n([\s\S]*?)\n---/);
  if (!section) {
...
export function validateCondition(entry, label, errors) {
...
export function validateEligibility(eligibility, errors) {
...
export function validateBenefit(benefit, errors, ratePolicyValues) {
...
export function validateCostBasis(costBasis, label, errors) {
...
// §3.1 term-type table.
export function validateTerm(term, label, errors) {
...
export function validateSubsidyRule(rule, errors) {
...
export function validateScheme(scheme, ratePolicyValues) {
