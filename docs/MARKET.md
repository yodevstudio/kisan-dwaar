...
- **Farmers onboarded to RajkisanSaathi.** **98 lakh farmers** onboarded to the state's RajkisanSaathi portal (*Rajasthan Economic Review 2025-26*). This is the closest published proxy for "farmers the state itself already has an active, addressable digital touchpoint for" — the population a Rajasthan-agriculture registry needs to reach is at minimum this large.
- **AgriStack Farmer IDs generated in Rajasthan.** **78,37,621 Farmer IDs** generated as of 14 August 2025 (Digital Sansad, 2025 parliamentary answer on AgriStack). Unlike RajkisanSaathi's onboarding count, a Farmer ID is tied to a specific land parcel and identity record, so this is the more precise denominator for "individually verified farmers this registry's eligibility rules could, in principle, be pre-filled against" once AgriStack and this registry are connected — a Phase-3-or-later integration, not something this project does today (`docs/ROADMAP.md`).
- **Mini e-Mitra operators.** **25,000 Mini e-Mitra operators**, per the *Rajasthan Budget 2026-27*'s Next Generation Citizen Service Reforms. The extension-worker/operator mode this project ships (`admin/operator/`) is designed to run from exactly this kind of kiosk — 25,000 is the number of existing, staffed access points a Rajasthan-wide rollout would not need to build, only reach.

These three published figures give a defensible denominator for "how many farmers and how many kiosks a Rajasthan-agriculture-only deployment already reaches" — the base of the ladder, not its ceiling. `[NEEDS SOURCE: each figure above is stated as given; before it appears in a deck or a submitted document, confirm it directly against the primary document named — the Rajasthan Economic Review 2025-26, the cited Digital Sansad parliamentary answer, and the Rajasthan Budget 2026-27 — rather than this file's transcription of it.]`
