...
- **Cost to run:** near-zero per transaction, because the eligibility engine runs in the citizen's own browser against static JSON, not a server queried per citizen — measured at ~75 KB gzipped for the full critical path (`docs/audit-evidence/K14_HARDENING_AUDIT.md`), which on Netlify's published free-tier bandwidth alone supports roughly 1.4 million first-time page loads a month before any hosting cost is incurred at all.
...
- The K14 page-weight measurement this document's financial model cites predates later visual and portal-structure passes; the true current figure will differ modestly and should be re-measured before being quoted precisely.
